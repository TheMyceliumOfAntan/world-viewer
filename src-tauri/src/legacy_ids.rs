//! Legacy (1.12 and earlier) block id -> name tables.
//!
//! Chunks before 1.13 store numeric block ids. Their names come from
//! `level.dat`, but that only works for 1.7.10 (which has `FML.ItemData`).
//! From 1.12 the registry moved to `FML.Registries.minecraft:blocks.ids` and
//! only lists *modded* blocks, so vanilla ids must be known here.

/// Vanilla block ids for Minecraft 1.12.2 (flattening happened in 1.13).
pub const VANILLA_1_12_2: &[(u16, &str)] = &[
    (0, "minecraft:air"),
    (1, "minecraft:stone"),
    (2, "minecraft:grass"),
    (3, "minecraft:dirt"),
...
/// Vanilla block ids for Minecraft 1.7.10, used when `level.dat` has no
/// `FML.ItemData` (e.g. a vanilla 1.7.10 save).
pub const VANILLA_1_7_10: &[(u16, &str)] = &[
    (0, "minecraft:air"),
    (1, "minecraft:stone"),
    (2, "minecraft:grass"),
    (3, "minecraft:dirt"),
